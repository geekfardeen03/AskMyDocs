...
# chose BAAI/bge over MiniLM because it scored better on my test queries
# 768 dims vs 384 — worth the extra memory for better retrieval
...
class Embedder:
    def __init__(self):
        ...
        self.index  = None
        self.chunks = []   # parallel list to FAISS — chunk[i] matches vector[i]
        print("Model ready.")
    ...
    def build_index(self, chunks):
        ...
        Using IndexFlatIP (exact search with inner product) — fine for <50k chunks.
        For larger sets I'd switch to IVF but this keeps it simple.
        """
        ...
        self.save()
        print(f"Index built: {self.index.ntotal} vectors saved.")
    ...
    def load(self):
        ...
        print(f"Loaded index: {self.index.ntotal} vectors, {len(set(c['filename'] for c in self.chunks))} docs")
        return True

    @property
    def total_docs(self):
        return len(set(c["filename"] for c in self.chunks)) if self.chunks else 0